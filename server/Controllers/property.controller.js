...
const getAllProprties = async (req, res) =>{
...
try {
   const count = await Property.countDouments({query});
...
const photourl = await cloudinary.uploder.upload (photo);
...
try {
   ...
   res.status(200).jason({message: 'prototype Updated Successfully'})
} catch (error) {
   res.status(500).json({message : error.message})
}
